...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM sys.columns WHERE object_id = OBJECT_ID('PLS.QueryMaster') AND name = 'DatabaseName')
    BEGIN
        ALTER TABLE [PLS].[QueryMaster] ADD [DatabaseName] VARCHAR(100) NULL;
    END

    -- Add SchemaName column if it doesn't exist
    IF NOT EXISTS (SELECT 1 FROM sys.columns WHERE object_id = OBJECT_ID('PLS.QueryMaster') AND name = 'SchemaName')
    BEGIN
        ALTER TABLE [PLS].[QueryMaster] ADD [SchemaName] VARCHAR(100) NULL;
    END

    -- Add TableOrViewName column if it doesn't exist
    IF NOT EXISTS (SELECT 1 FROM sys.columns WHERE object_id = OBJECT_ID('PLS.QueryMaster') AND name = 'TableOrViewName')
    BEGIN
        ALTER TABLE [PLS].[QueryMaster] ADD [TableOrViewName] VARCHAR(150) NULL;
    END
END
GO
...
BEGIN
    ...
    VALUES ('purchasing_po_header', N'Get Purchase Orders', N'[PLS].[APIPlusOperation]', 'GetPurchaseOrders', N'Retrieve purchase order header list', 
            N'SELECT PurchaseOrderNumber, EnteredDate, VendorNumber, TotalAmount, OrderState, VendorName FROM QGetPurchaseOrders WHERE EnteredDate BETWEEN @FromDate AND @ToDate ORDER BY PurchaseOrderNumber DESC;', 'ERPMega', 'dbo', 'QGetPurchaseOrders', 'System');
END
ELSE
BEGIN
...
BEGIN
    ...
    VALUES ('purchasing_po_header', N'Get Purchase Order Lines', N'[PLS].[APIPlusOperation]', 'GetPurchaseOrderLines', N'Retrieve purchase order line item details', 
            N'SELECT Line, PurchasedCode AS ItemCode, QuantityOrdered AS OrderedQuantity, Price AS UnitPrice, LineAmount AS TotalAmount FROM QGetPurchaseOrderLines WHERE OrderNumber = @PONumber ORDER BY Line;', 'ERPMega', 'dbo', 'QGetPurchaseOrderLines', 'System');
END
ELSE
BEGIN
    UPDATE [PLS].[QueryMaster]
...
BEGIN
    ...
    VALUES ('purchasing_po_header', N'Get Vendors', N'[PLS].[APIPlusOperation]', 'GetVendors', N'Retrieve vendor master details', 
            N'SELECT VendorNumber, VendorName FROM QGetVendors ORDER BY VendorName;', 'ERPMega', 'dbo', 'QGetVendors', 'System');
END
ELSE
BEGIN
    UPDATE [PLS].[QueryMaster]
...
BEGIN
    ...
    VALUES ('purchasing_po_line', N'Get Items', N'[PLS].[APIPlusOperation]', 'GetItems', N'Retrieve items list from master', 
            N'SELECT ItemCode, ItemDescription FROM QGetItems ORDER BY ItemCode;', 'ERPMega', 'dbo', 'QGetItems', 'System');
END
ELSE
BEGIN
    UPDATE [PLS].[QueryMaster]
...
BEGIN
    ...
    VALUES ('safety_stock_item_master', N'Get Safety Stock Items', N'[PLS].[APIPlusOperation]', 'GetSaftyStockItems', N'Retrieve active items monitored for safety stock', 
            N'SELECT ID, ItemCode, SaftyStock, LeadTime, ServiceLevelFactor, ItemType FROM QGetSaftyStockItems ORDER BY ItemCode;', 'ERPMega', 'dbo', 'QGetSaftyStockItems', 'System');
END
ELSE
BEGIN
    UPDATE [PLS].[QueryMaster]
...
BEGIN
    ...
    VALUES ('safety_stock_item_master', N'Get Item Balance', N'[PLS].[APIPlusOperation]', 'GetItemBalance', N'Retrieve on-hand warehouse inventory balance', 
            N'SELECT ItemCode, ItemBalance AS OnHandBalance FROM QGetItemBalance WHERE ItemCode = @ItemCode;', 'ERPMega', 'dbo', 'QGetItemBalance', 'System');
END
ELSE
BEGIN
    UPDATE [PLS].[QueryMaster]
...
BEGIN
    ...
    VALUES ('safety_stock_item_master', N'Get Item Consumption', N'[PLS].[APIPlusOperation]', 'GetItemConsumption', N'Retrieve item consumption usage history logs', 
            N'SELECT ItemCode, TotalQuantity AS ConsumptionQty, CAST(CAST(Yer AS VARCHAR(4)) + ''-'' + CAST(Mnth AS VARCHAR(2)) + ''-01'' AS DATE) AS ConsumptionDate FROM QGetItemConsumption WHERE ItemCode = @ItemCode ORDER BY ConsumptionDate DESC;', 'ERPMega', 'dbo', 'QGetItemConsumption', 'System');
END
ELSE
BEGIN
    UPDATE [PLS].[QueryMaster]
...
BEGIN
    ...
    VALUES ('safety_stock_item_master', N'Get Item Open POs', N'[PLS].[APIPlusOperation]', 'GetItemOpenPOs', N'Retrieve outstanding open purchase orders for an item', 
            N'SELECT OrderNumber AS PurchaseOrderNumber, QuantityOrdered AS OrderedQuantity, QuantityReceived AS RecievedQuantity FROM QGetItemOpenPOs WHERE ItemCode = @ItemCode AND QuantityReceived < QuantityOrdered;', 'ERPMega', 'dbo', 'QGetItemOpenPOs', 'System');
END
ELSE
BEGIN
    UPDATE [PLS].[QueryMaster]
...
BEGIN
    ...
    VALUES ('safety_stock_item_master', N'Get Item Lead Time', N'[PLS].[APIPlusOperation]', 'GetItemLeadTime', N'Retrieve delivery lead times from supplier logs', 
            N'SELECT ItemCode, LeadTime AS SupplierLeadTime FROM QGetItemLeadTime WHERE ItemCode = @ItemCode;', 'ERPMega', 'dbo', 'QGetItemLeadTime', 'System');
END
ELSE
BEGIN
    UPDATE [PLS].[QueryMaster]
...
BEGIN
    ...
    VALUES ('safety_stock_item_master', N'Get Item Status History', N'[PLS].[APIPlusOperation]', 'GetItemStatusHistory', N'Retrieve status transition logs for an item', 
            N'SELECT LogID, ItemCode, OldStatus, NewStatus, LogDate FROM QGetItemStatusHistory WHERE ItemCode = @ItemCode ORDER BY LogDate DESC;', 'ERPMega', 'dbo', 'QGetItemStatusHistory', 'System');
END
ELSE
BEGIN
    UPDATE [PLS].[QueryMaster]
...
BEGIN
    ...
    VALUES ('safety_stock_item_master', N'Get Item Receipts', N'[PLS].[APIPlusOperation]', 'GetItemReceipts', N'Retrieve stock receipt history logs for an item', 
            N'SELECT ItemCode, QuantityReceived AS ReceiptQuantity, ReceivingDate AS ReceiptDate FROM QGetItemReceipts WHERE ItemCode = @ItemCode ORDER BY ReceiptDate DESC;', 'ERPMega', 'dbo', 'QGetItemReceipts', 'System');
END
ELSE
BEGIN
    UPDATE [PLS].[QueryMaster]
...
BEGIN
    ...
    VALUES ('safety_stock_item_master', N'Save Safety Stock Item', N'[PLS].[APIPlusOperation]', 'SaveSaftyStockItem', N'Insert or update safety stock settings for an item', 
            N'MERGE PUR.SaftyStockItemMaster AS t USING (SELECT @ItemID AS ID) AS s ON t.ID = s.ID WHEN MATCHED THEN UPDATE SET SaftyStock = @SaftyStock, LeadTime = @LeadTime WHEN NOT MATCHED THEN INSERT (ItemCode, SaftyStock, LeadTime) VALUES (@ItemCode, @SaftyStock, @LeadTime);', 'ERPMega', 'PUR', 'SaftyStockItemMaster', 'System');
END
ELSE
BEGIN
    UPDATE [PLS].[QueryMaster]
...
BEGIN
    ...
    VALUES ('user_permissions', N'Get System Users', N'[PLS].[APIPlusOperation]', 'GetSystemUsers', N'Retrieve system users along with group names', 
            N'SELECT a.Username, a.Name, a.IsAdmin, b.GroupName FROM ERPManagement.[System].[UserMaster] a LEFT OUTER JOIN ERPManagement.[System].GroupMaster b ON a.GroupID = b.GroupID ORDER BY a.Username;', 'ERPManagement', 'System', 'UserMaster', 'System');
END
ELSE
BEGIN
    UPDATE [PLS].[QueryMaster]
...
BEGIN
    ...
    VALUES ('user_permissions', N'Get Pages and Groups', N'[PLS].[APIPlusOperation]', 'GetPagesAndGroups', N'Retrieve application pages and group metadata list', 
            N'SELECT PageGroupID, Label, Icon, Description, IsGroup, ParentID, SortOrder FROM [PLS].[PagesAndGroups] ORDER BY SortOrder;', 'ERPMega', 'PLS', 'PagesAndGroups', 'System');
END
ELSE
BEGIN
    UPDATE [PLS].[QueryMaster]
...
BEGIN
    ...
    VALUES ('user_permissions', N'Get User Page Permissions', N'[PLS].[APIPlusOperation]', 'GetUserPagePermissions', N'Retrieve permission mapping records list', 
            N'SELECT p.PermissionID, p.Username, p.PageGroupID, pg.Label AS PageLabel, p.CanView FROM [PLS].[UserPagePermissions] p INNER JOIN [PLS].[PagesAndGroups] pg ON p.PageGroupID = pg.PageGroupID ORDER BY p.Username;', 'ERPMega', 'PLS', 'UserPagePermissions', 'System');
END
ELSE
BEGIN
    UPDATE [PLS].[QueryMaster]
