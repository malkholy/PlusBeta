...
BEGIN
    ...
    UPDATE [PLS].[QueryMaster]
    SET [QuerySQL] = N'SELECT LHID, TrackNumber, TrackState, VendorNumber, BankNumber, ForwarderID, PINumber, ShipmentState, AccountingState, DocumentState, Currency, ETA, ETD, InvoiceNumber, Destination, LogisitcNote, AttachmentID, CustomsBrokerRef, RequestShippingDate, OfficeCourierArrivalDate, BankCourierArrivalDate, SentToBankDate, ReleasedFromBankDate, FactoryArrivalDate, PaymentTermID, IncoTermID, IsLocked, CarrierID, LogisticCreatedBy, LogisticCreatedDate, LogisticLastMaintBy, LogisticLastMaintDate, ClearingAgentID, ForwarderName, CarrierName, ClearingAgentName, PaymentTermDescription, IncoTermDescription, StateDescription, ShipmentStateDescription, DocumentStateDescription, AccountingStateDescription, PaymentStateDescription, VendorName, VendorExtraName, BankAccountName, ItemAmount, DiscountAmount, FreightAmount, InsuranceAmount, TotalAmount, ACINumber, BLNumber, BLType, ShipmentMode, ShipmentSize, AssignToUser, CertificateNo, PONumber FROM QGetTrackingHistory ORDER BY LogisticCreatedDate DESC, TrackNumber DESC;',
...
BEGIN
    ...
    VALUES (N'Get Track Details Header', N'[dbo].[APIPlusLogisticsOperation]', 'GetTrackingHistory', N'Retrieve header detail record for a track', 
            N'SELECT LHID, TrackNumber, TrackState, VendorNumber, BankNumber, ForwarderID, PINumber, ShipmentState, AccountingState, DocumentState, Currency, ETA, ETD, InvoiceNumber, Destination, LogisitcNote, AttachmentID, CustomsBrokerRef, RequestShippingDate, OfficeCourierArrivalDate, BankCourierArrivalDate, SentToBankDate, ReleasedFromBankDate, FactoryArrivalDate, PaymentTermID, IncoTermID, IsLocked, CarrierID, LogisticCreatedBy, LogisticCreatedDate, LogisticLastMaintBy, LogisticLastMaintDate, ClearingAgentID, ForwarderName, CarrierName, ClearingAgentName, PaymentTermDescription, IncoTermDescription, StateDescription, ShipmentStateDescription, DocumentStateDescription, AccountingStateDescription, VendorName, VendorExtraName, BankAccountName, ItemAmount, DiscountAmount, FreightAmount, InsuranceAmount, TotalAmount, ACINumber, BLNumber, BLType, ShipmentMode, ShipmentSize, AssignToUser, CertificateNo, PONumber FROM QGetTrackingHistory ORDER BY LogisticCreatedDate DESC, TrackNumber DESC;', 'ERPMega', 'dbo', 'QGetTrackingHistory', 'Grid', 'System');
    SET @QID = SCOPE_IDENTITY();
END
ELSE
BEGIN
    ...
    UPDATE [PLS].[QueryMaster]
    SET [QuerySQL] = N'SELECT LHID, TrackNumber, TrackState, VendorNumber, BankNumber, ForwarderID, PINumber, ShipmentState, AccountingState, DocumentState, Currency, ETA, ETD, InvoiceNumber, Destination, LogisitcNote, AttachmentID, CustomsBrokerRef, RequestShippingDate, OfficeCourierArrivalDate, BankCourierArrivalDate, SentToBankDate, ReleasedFromBankDate, FactoryArrivalDate, PaymentTermID, IncoTermID, IsLocked, CarrierID, LogisticCreatedBy, LogisticCreatedDate, LogisticLastMaintBy, LogisticLastMaintDate, ClearingAgentID, ForwarderName, CarrierName, ClearingAgentName, PaymentTermDescription, IncoTermDescription, StateDescription, ShipmentStateDescription, DocumentStateDescription, AccountingStateDescription, VendorName, VendorExtraName, BankAccountName, ItemAmount, DiscountAmount, FreightAmount, InsuranceAmount, TotalAmount, ACINumber, BLNumber, BLType, ShipmentMode, ShipmentSize, AssignToUser, CertificateNo, PONumber FROM QGetTrackingHistory ORDER BY LogisticCreatedDate DESC, TrackNumber DESC;',
        [SPName] = N'[dbo].[APIPlusLogisticsOperation]',
        [DatabaseName] = 'ERPMega',
        [SchemaName] = 'dbo',
        [TableOrViewName] = 'QGetTrackingHistory',
...
-- 28. Get Track Details Attachments
IF NOT EXISTS (SELECT 1 FROM [PLS].[QueryMaster] WHERE [QueryName] = N'Get Track Details Attachments')
BEGIN
    INSERT INTO [PLS].[QueryMaster] ([QueryName], [SPName], [Operation], [Description], [QuerySQL], [DatabaseName], [SchemaName], [TableOrViewName], [QueryType], [CreatedBy])
    VALUES (N'Get Track Details Attachments', N'[dbo].[APIPlusLogisticsOperation]', 'GetTrackingHistoryAttachments', N'Retrieve attachments for a track', 
            N'SELECT * FROM QGetTrackDetailsAttachments WHERE TrackNumber = @TrackNumber;', 'ERPMega', 'dbo', 'QGetTrackDetailsAttachments', 'Grid', 'System');
    SET @QID = SCOPE_IDENTITY();
END
ELSE
BEGIN
    SELECT @QID = QueryID FROM [PLS].[QueryMaster] WHERE [QueryName] = N'Get Track Details Attachments';
    UPDATE [PLS].[QueryMaster]
    SET [QuerySQL] = N'SELECT * FROM QGetTrackDetailsAttachments WHERE TrackNumber = @TrackNumber;',
        [SPName] = N'[dbo].[APIPlusLogisticsOperation]',
        [DatabaseName] = 'ERPMega',
        [SchemaName] = 'dbo',
        [TableOrViewName] = 'QGetTrackDetailsAttachments',
        [QueryType] = 'Grid'
    WHERE QueryID = @QID;
END
IF NOT EXISTS (SELECT 1 FROM [PLS].[PageQueries] WHERE PageGroupID = 'logistics_track_details' AND QueryID = @QID)
